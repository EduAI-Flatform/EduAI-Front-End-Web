...
async function installFixtures(page: import('@playwright/test').Page) {
  ...
  await page.route('**/api/v1/membership/catalog', (route) => route.fulfill(json({ items: [
    ...
    { id: 'basic-version', plan: { id: 'basic-plan', code: 'BASIC' }, displayName: 'EduAI Basic', description: null, currency: 'VND', durations: [{ id: 'basic-monthly', months: 1, basePriceAmountMinor: '100000', discountPercent: 0, finalPriceAmountMinor: '100000' }], services: [], includedCourses: [] },
  ] })));
  await page.route('**/api/v1/membership/current', (route) => route.fulfill(json({ membership: { id: 'subscription-id', plan: { id: 'gold-plan', code: 'GOLD' }, versionId: 'gold-version', displayName: 'EduAI Gold', startsAt: '2026-08-01T00:00:00.000Z', expiresAt: '2027-08-01T00:00:00.000Z', status: 'ACTIVE' }, pendingChange: null })));

...
import { getJobError, jobService, type Job, type JobMatch } from "../../services/job.service";
...
export function JobDetailPage() {
  ...
  const [match, setMatch] = useState<JobMatch | null>(null); const [matchError, setMatchError] = useState(""); const [matchLoading, setMatchLoading] = useState(false);
  ...
  useEffect(() => { if (!canApply) return; let active = true; setMatchLoading(true); jobService.match(jobId).then((value) => { if (active) { setMatch(value); setMatchError(""); } }).catch((reason) => { if (active) setMatchError(getJobError(reason)); }).finally(() => { if (active) setMatchLoading(false); }); return () => { active = false; }; }, [canApply, jobId]);

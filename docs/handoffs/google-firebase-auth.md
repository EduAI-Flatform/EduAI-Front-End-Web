...
- Firebase Auth is initialized with Firebase's supported `getAuth(firebaseApp)` browser defaults. Do not rebuild its popup/redirect dependency graph with `initializeAuth`; a custom resolver configuration caused the production popup to fail before Google authorization. If an in-app browser (Zalo, Facebook, Messenger, or Android WebView) cannot return a popup to the initiating page, the UI keeps the user in EduAI and offers a normal-browser reopen, retry, and home action.
...
- The backend must return an exact CORS allow-origin match for every deployed frontend origin. `CORS_ALLOWED_ORIGINS` is a comma-separated, validated origin-only allowlist; the Render blueprint pins `https://eduai.giaoducso.org.vn`. A path, query, fragment, credentials, or wildcard is rejected.
...
- OAuth diagnostics report only a safe classification, stage (`authorization`, `callback`, `token_exchange`, or `session`), bounded provider/backend code, HTTP status, and correlation ID. Provider messages, authorization codes, Firebase ID tokens, refresh/access tokens, secrets, and session identifiers must never be emitted.
- The service worker treats Firebase helpers, auth pages/callbacks, API routes, and authenticated/private routes as network-only. These paths intentionally fail closed offline.
- Production release verification is incomplete until a human selects a Google account and completes consent, the Firebase ID token exchange succeeds, an EduAI session is stored, the authenticated redirect succeeds, and refresh/logout/relogin/cancel/error behavior is checked. Never use fabricated credentials to bypass this gate.
- Supplemental Playwright WebKit 26.5 verification on Firebase's authorized `localhost` origin, using the production auth domain, opened the popup and reached Google authorization without emitting a sanitized failure event. This is engine-level pre-consent evidence only, not physical Safari, ITP, callback, or EduAI-session proof.
